...
def get_audios():
    ...
    folder = f'{target_date_str}_korean.mp3'
...
@app.route("/api/news_global")
def send_news_global():
    """
    전날 글로벌 뉴스 데이터 가져오기
    """
    ...
    return jsonify({"news_global": news_items_global})


@app.route("/startups/global")
def send_startups_global():
    """
    전날 뉴스 데이터 가져오기
    """
    ...
    client = MongoClient(host=db_info['my_ip'], port=27017,
                         username=db_info['username'], password=db_info['password'])
    db = client[db_info['db_name']]
    collection = db[db_info['collection_name_startups_global']]

    startups_items = list(collection.find({}, {'_id': False}))
    # print(news_items)

    return jsonify({"startups": startups_items})
